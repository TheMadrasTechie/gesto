"""
Gesto — data collection (fixed frame count).

Records gesture samples of a FIXED length set by NUM_FRAMES in gesto_common.py
(10, 20, or 30). Press SPACE to record a sample; it captures exactly NUM_FRAMES
frames automatically, then waits for the next one.
...
Controls:
    SPACE  start recording the next sample (captures NUM_FRAMES frames)
    q      quit

Files:
    gesture_data/<class>/<sample_index>.npy     # shape (NUM_FRAMES, 63)
"""
        ...
        while cap.isOpened() and sample_index < SAMPLES_PER_CLASS:
            ...
            if recording:
                ...
                banner(frame, f"class '{class_name}'  next sample "
                              f"{sample_index + 1}/{SAMPLES_PER_CLASS}")
                banner(frame, f"SPACE = record {NUM_FRAMES} frames    q = quit",
                       y=70, color=(200, 200, 200))
