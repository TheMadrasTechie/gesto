...
Loads all collected samples from gesture_data/, trains an LSTM, and saves:
    gesto_model.h5      the trained model
    labels.json         class index -> class name (the numbers you collected)

Each sample is (30, 63): 30 frames x 63 hand-landmark features.
...
def load_data():
    """Load every <class>/<n>.npy into X (n,30,63) and integer labels."""
    classes = sorted(
    ...
    for name in classes:
        ...
        for f in files:
            ...
            if arr.shape != (FRAMES_PER_SAMPLE, FEATURE_DIM):
...
def build_model(n_classes):
    """LSTM matching the (30,63) input. tanh activations keep training stable
    and let Keras use the fast cuDNN LSTM kernel."""
    model = Sequential([
        LSTM(64, return_sequences=True, activation="tanh", input_shape=(30, 63)),
        Dropout(0.3),
...
def main():
    ...
    # stratify so each class appears in both train and validation
    ...
    # save index -> name so display code can map numbers to letters later
