"""
Gesto full-body pose — train.

Self-contained train script for the pose region (dim 132).
...
def main():
    ap = argparse.ArgumentParser(description="Train pose gesture model")
    ...
    ap.add_argument("--out", default="artifacts_pose")
    args = ap.parse_args()

    X, y, labels = load(args.project_dir, args.seq_len)
    print(f"pose (dim 132): {len(X)} sequences, classes={labels}")
