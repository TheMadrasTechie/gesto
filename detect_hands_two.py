"""
Gesto two hands — detect.

Self-contained detect script for the hands_two region (dim 126).
...
def extract(res):
    """Return the hands_two feature vector (raw, unnormalized)."""
...
def main():
    ap = argparse.ArgumentParser(description="Live hands_two gesture detection")
    ap.add_argument("artifacts", nargs="?", default="artifacts_hands_two",
    ...
    print(f"hands_two model: {labels} (T={T}, normalize={use_norm})")
    ...
    while cap.isOpened():
        ...
        cv2.imshow("Gesto hands_two detect (q=quit)", frame)
