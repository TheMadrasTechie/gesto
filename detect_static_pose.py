"""
Gesto full-body pose — STATIC detect (single frame, no motion).

Classifies EVERY FRAME on its own — no 30-frame window, no warm-up, instant
response. Use with a model from train_static_pose.py.
...
Run:
    ...
    python detect_static_pose.py artifacts_static_hands_one
    python detect_static_pose.py artifacts_static_hands_one --source clip.mp4
...
def extract(res):
    """33 pose points x (x,y,z,visibility). None if no body detected."""
    if not res.pose_landmarks:
        return None
    return np.array([[r.x, r.y, r.z, r.visibility]
                     for r in res.pose_landmarks.landmark],
                    dtype=np.float32).flatten()


# ---------- normalization (EXACT copy of Gesto's normalize_vector) ----------
def normalize(vec):
    pts = np.asarray(vec, np.float32).reshape(33, 4).copy()
    xyz = pts[:, :3]
...
def main():
    ap = argparse.ArgumentParser(description="Live static pose detection")
    ap.add_argument("artifacts", nargs="?", default="artifacts_static_pose",
    ...
    win = "Gesto static pose detect (q=quit)"
