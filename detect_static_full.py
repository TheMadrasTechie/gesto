"""
Gesto full body + hands — STATIC detect (single frame, no motion).

Classifies EVERY FRAME on its own — no 30-frame window, no warm-up, instant
response. Use with a model from train_static_full.py.
...
Run:
    python detect_static_full.py
    python detect_static_full.py artifacts_static_hands_one
    python detect_static_full.py artifacts_static_hands_one --source clip.mp4
...
def _pose(res):
    return (np.array([[r.x, r.y, r.z, r.visibility]
                      for r in res.pose_landmarks.landmark]).flatten()
            if res.pose_landmarks else np.zeros(132, np.float32))

def extract(res):
    """pose(132) + left hand(63) + right hand(63) = 258. None if nothing seen."""
    if not (res.pose_landmarks or res.left_hand_landmarks or res.right_hand_landmarks):
        return None
    return np.concatenate([_pose(res), _lh(res), _rh(res)]).astype(np.float32)
...
def main():
    ap = argparse.ArgumentParser(description="Live static full detection")
    ap.add_argument("artifacts", nargs="?", default="artifacts_static_full",
    ...
    win = "Gesto static full detect (q=quit)"
    ...
    while cap.isOpened():
        ...
        hint = "" if kp is not None else "  (nothing detected)"
