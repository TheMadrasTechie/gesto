"""
Gesto two hands — STATIC detect (single frame, no motion).

Classifies EVERY FRAME on its own — no 30-frame window, no warm-up, instant
response. Use with a model from train_static_hands_two.py.
...
Run:
    python detect_static_hands_two.py
    python detect_static_hands_two.py artifacts_static_hands_one
    python detect_static_hands_two.py artifacts_static_hands_one --source clip.mp4
...
def extract(res):
    """Left hand then right hand — exactly like Gesto. None if neither seen."""
    if not (res.left_hand_landmarks or res.right_hand_landmarks):
        return None
    return np.concatenate([_lh(res), _rh(res)]).astype(np.float32)
...
def normalize(vec):
    pts = np.asarray(vec, np.float32).reshape(2, 21, 3).copy()
    for h in range(2):
...
def main():
    ap = argparse.ArgumentParser(description="Live static hands_two detection")
    ap.add_argument("artifacts", nargs="?", default="artifacts_static_hands_two",
    ...
    win = "Gesto static hands_two detect (q=quit)"
    ...
    while cap.isOpened():
        ...
        hint = "" if kp is not None else "  (no hands)"
