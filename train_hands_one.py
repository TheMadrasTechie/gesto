"""
Gesto one hand — train.

Self-contained train script for the hands_one region (dim 63).
...
def main():
    ap = argparse.ArgumentParser(description="Train hands_one gesture model")
    ...
    ap.add_argument("--out", default="artifacts_hands_one")
    args = ap.parse_args()

    X, y, labels = load(args.project_dir, args.seq_len)
    print(f"hands_one (dim 63): {len(X)} sequences, classes={labels}")
