...
Self-contained detect script for the legs region (dim 32).
...
def extract(res):
    """Return the legs feature vector (raw, unnormalized)."""
    return _pose_legs(res).astype(np.float32)
...
def main():
    ap = argparse.ArgumentParser(description="Live legs gesture detection")
    ap.add_argument("artifacts", nargs="?", default="artifacts_legs",
    ...
    print(f"legs model: {labels} (T={T}, normalize={use_norm})")
    ...
    while cap.isOpened():
        ...
        cv2.imshow("Gesto legs detect (q=quit)", frame)
