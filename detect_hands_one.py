"""
Gesto one hand — detect.

Self-contained detect script for the hands_one region (dim 63).
Reads Gesto Labeller data captured with region="Hands" and hands="one".
...
def extract(res):
    """Return the hands_one feature vector (raw, unnormalized)."""
    hand = res.right_hand_landmarks or res.left_hand_landmarks   # prefer right, else left
    if hand is None:
        return np.zeros(63, np.float32)
    return np.array([[r.x, r.y, r.z] for r in hand.landmark]).flatten().astype(np.float32)
...
def main():
    ap = argparse.ArgumentParser(description="Live hands_one gesture detection")
    ap.add_argument("artifacts", nargs="?", default="artifacts_hands_one",
    ...
    print(f"hands_one model: {labels} (T={T}, normalize={use_norm})")
    ...
    while cap.isOpened():
        ...
        cv2.imshow("Gesto hands_one detect (q=quit)", frame)
