"""
Gesto legs — STATIC detect (single frame, no motion).

Classifies EVERY FRAME on its own — no 30-frame window, no warm-up, instant
response. Use with a model from train_static_legs.py.
...
Run:
    python detect_static_legs.py
    python detect_static_legs.py artifacts_static_hands_one
    python detect_static_legs.py artifacts_static_hands_one --source clip.mp4
...
def extract(res):
    """8 lower-body points x (x,y,z,visibility). None if no body detected."""
    if not res.pose_landmarks:
        return None
    lm = res.pose_landmarks.landmark
    return np.array([[lm[i].x, lm[i].y, lm[i].z, lm[i].visibility]
                     for i in LEG_POSE_IDX], dtype=np.float32).flatten()


# ---------- normalization (EXACT copy of Gesto's normalize_vector) ----------
def normalize(vec):
    pts = np.asarray(vec, np.float32).reshape(len(LEG_POSE_IDX), 4).copy()
    xyz = pts[:, :3]
...
def main():
    ap = argparse.ArgumentParser(description="Live static legs detection")
    ap.add_argument("artifacts", nargs="?", default="artifacts_static_legs",
    ...
    win = "Gesto static legs detect (q=quit)"
