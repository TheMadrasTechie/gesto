"""
Gesto two hands — train.

Self-contained train script for the hands_two region (dim 126).
...
def main():
    ap = argparse.ArgumentParser(description="Train hands_two gesture model")
    ...
    ap.add_argument("--out", default="artifacts_hands_two")
    args = ap.parse_args()

    X, y, labels = load(args.project_dir, args.seq_len)
    print(f"hands_two (dim 126): {len(X)} sequences, classes={labels}")
