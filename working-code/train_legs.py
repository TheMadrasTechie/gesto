...
Self-contained train script for the legs region (dim 32).
...
def main():
    ap = argparse.ArgumentParser(description="Train legs gesture model")
    ...
    ap.add_argument("--out", default="artifacts_legs")
    args = ap.parse_args()

    X, y, labels = load(args.project_dir, args.seq_len)
    print(f"legs (dim 32): {len(X)} sequences, classes={labels}")
