"""
Gesto full-body pose — STATIC train (single frame, no motion).
...
in STATIC mode:   <project>/data/static/<label>/<uid>.npy   each of shape (132,)

Everything is open source: MediaPipe (Apache-2.0), TensorFlow (Apache-2.0),
OpenCV (Apache-2.0), NumPy (BSD).

Run:
    python train_static_pose.py "D:\\...\\gesto_projects\\my-project"
    python train_static_pose.py <project> --epochs 200 --out artifacts_static_pose
"""
    ...
    for name in labels:
        for f in sorted((root / name).glob("*.npy")):
            ...
            if a.shape[0] != FEATURE_DIM:
                sys.exit(f"{f} has dim {a.shape[0]} but pose expects "
...
def build(n_classes, small):
    """Dense classifier for a single 132-value landmark vector.
...
def main():
    ap = argparse.ArgumentParser(description="Train static pose gesture model")
    ...
    ap.add_argument("--out", default="artifacts_static_pose")
    args = ap.parse_args()

    X, y, labels = load(args.project_dir)
    print(f"static pose (dim {FEATURE_DIM}): {len(X)} samples, classes={labels}")
