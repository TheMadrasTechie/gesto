"""
Gesto one hand — STATIC train (single frame, no motion).
...
Reads Gesto Labeller data captured with region="Hands", hands="one",
in STATIC mode:   <project>/data/static/<label>/<uid>.npy   each of shape (63,)

Everything is open source: MediaPipe (Apache-2.0), TensorFlow (Apache-2.0),
OpenCV (Apache-2.0), NumPy (BSD).

Run:
    python train_static_hands_one.py "D:\\...\\gesto_projects\\hand-signs"
    python train_static_hands_one.py <project> --epochs 200 --out artifacts_static_hands_one
"""
    ...
    for name in labels:
        for f in sorted((root / name).glob("*.npy")):
            ...
            if a.shape[0] != FEATURE_DIM:
                sys.exit(f"{f} has dim {a.shape[0]} but hands_one expects "
...
def build(n_classes, small):
    """Dense classifier for a single 63-value landmark vector.
...
def main():
    ap = argparse.ArgumentParser(description="Train static one-hand gesture model")
    ...
    ap.add_argument("--out", default="artifacts_static_hands_one")
    args = ap.parse_args()

    X, y, labels = load(args.project_dir)
    print(f"static hands_one (dim {FEATURE_DIM}): {len(X)} samples, classes={labels}")
